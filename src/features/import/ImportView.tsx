  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <motion.div
        ...
        className="p-8 rounded-3xl bg-slate-900/60 border-2 border-dashed border-indigo-500/30 hover:border-indigo-500/60 transition-all duration-300 text-center relative overflow-hidden shadow-xl"
      >
        <input
          type="file"
          ref={fileInputRef}
        ...
        <div className="flex flex-col items-center justify-center max-w-md mx-auto space-y-3">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-xs border border-slate-700 transition-all"
            >
                ...
                {mappings.map((mapItem) => (
                  ...
                  >
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="text-[10px] text-slate-500">Excel</span>
                    </div>

                    <select
                      value={mapItem.targetVariable}
                      onChange={(e) => handleMappingChange(mapItem.excelColumn, e.target.value)}
                      className="w-full bg-slate-900 text-slate-200 text-xs rounded-lg px-2.5 py-1.5 border border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                  ...
                  <tbody className="divide-y divide-slate-800/80">
                    {currentProject.students.slice(0, 10).map((std, idx) => (
                      <tr key={std.id} className="hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="p-3 font-mono text-indigo-400 font-medium">
                        ...
                        <td className="p-3 max-w-[180px] truncate text-slate-300">
                          {std.formation || <span className="text-rose-400 italic">Non spécifié</span>}
                        </td>
